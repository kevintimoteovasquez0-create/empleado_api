...
export class LicenciaMedicaController {
  ...
  @ApiOperation({
    ...
    description:
      'Retorna una lista paginada de licencias médicas. Se puede filtrar por estado.',
  })
  @ApiQuery({
    name: 'estado',
    required: false,
  ...
  findAllLicenciasMedicas(
    ...
    return this.licenciaMedicaService.findAllLicenciasMedicas(
      paginationDto,
      estadoValidado,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      'Retorna la información de una licencia médica específica según su ID.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
  ...
  findLicenciaMedicaById(
    ...
    const estadoValidado = estado ?? true;
    return this.licenciaMedicaService.findLicenciaMedicaById(
      id,
      estadoValidado,
    );
  }
  ...
  createLicenciaMedica(
    ...
    return this.licenciaMedicaService.createLicenciaMedica(
  ...
  updateLicenciaMedica(
    ...
    return this.licenciaMedicaService.updateLicenciaMedica(
      id,
      updateLicenciaMedicaDto,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      'Restaura una licencia médica que fue desactivada previamente.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
  ...
  restoreLicenciaMedica(@Param('id', ParseIntPipe) id: number) {
  ...
  removeLicenciaMedica(@Param('id', ParseIntPipe) id: number) {
    return this.licenciaMedicaService.removeLicenciaMedica(id);
  }

  @Patch(':id/estado')
  @ApiOperation({
    summary: 'Actualizar estado de licencia médica',
    description:
      'Actualiza el estado de una licencia médica (PENDIENTE, APROBADO, RECHAZADO).',
  })
  @ApiParam({
    name: 'id',
    type: Number,
    description: 'ID de la licencia médica a actualizar estado',
    example: 1,
  })
  @ApiBody({
    schema: {
  ...
  @ApiResponse({
    status: 200,
    description: 'Estado de licencia médica actualizado correctamente',
  })
  @ApiResponse({
    status: 404,
    description: 'Licencia médica no encontrada',
  ...
  updateEstado(
    @Param('id', ParseIntPipe) id: number,
    @Body() body: { estado: 'PENDIENTE' | 'APROBADO' | 'RECHAZADO' },
  ) {
    return this.licenciaMedicaService.updateEstado(id, body.estado);
  }
}
